...
kpi_checkbox = st.sidebar.checkbox('KPIs', value=False)
tops_checkbox = st.sidebar.checkbox('Top N', value=False)
viz_checkbox = st.sidebar.checkbox('Data Visualizations', value=False)
xray_checkbox = st.sidebar.checkbox('X-Ray', value=False)
...
if not kpi_checkbox and not tops_checkbox and not viz_checkbox and not xray_checkbox:
    st.header('Start by selecting one or many **Features** on the **Navigation** panel (sidebar)')
    session_break(2)
